...
parser.add_argument('--data_dir', type=str, default='/home/hc218/Zichen_Fan/data/cifar-10-python')
...
print("specify generative model finished")
...
print("specify discriminative model finished")
# costs
labels = T.ivector()
x_lab = T.tensor4()
x_unl = T.tensor4()
...
print("costs defination finished")
# test error
...
print("set Theano functions for training the disc net finished")
...
#gen_params_model = ll.get_all_params(gen_layers[-1])
gen_param_updates = nn.adam_updates(gen_params, loss_gen, lr=lr, mom1=0.5)
train_batch_gen = th.function(inputs=[lr], outputs=None, updates=gen_param_updates)
print("set Theano functions for training the gen net finished")
# select labeled data
...
for epoch in range(1200):
    ...
    for t in range(nr_batches_train):
        print("iteration=",(epoch*nr_batches_train+t+1))
    ...
    #print("test finished")
    # report
    with open(trainval_file, 'a') as f:
        f.write("Iteration %d, time = %ds, loss_lab = %.4f, loss_unl = %.4f, train err = %.4f, test err = %.4f\n" % (epoch, time.time()-begin, loss_lab, loss_unl, train_err, test_err))
    f.close()
    sys.stdout.flush()
    ...
    plotting.plt.savefig('./outputimage/cifar10_sample_minibatch'+str(epoch+1)+'.png')
    plotting.plt.close('all')
    #print(len(gen_layers))
    #print(len(gen_params))
    #print(len(gen_params_model))
    # save params
    if (epoch+1 % 100 == 0):
        np.savez('./model/disc_params'+str(epoch+1)+'.npz',*[p.get_value() for p in disc_params])
        np.savez('./model/gen_params'+str(epoch+1)+'.npz',*[p.get_value() for p in gen_params])
