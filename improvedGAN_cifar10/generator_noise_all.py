...
for i in range(int(number_output/batch_size)):
    ...
    with np.load('./model_old/gen_params300.npz') as f:
        param_values = [f['arr_%d' % k] for k in range(len(f.files))]
    #for k in range(len(param_values)):
    #    print(param_values[k].shape)
    if(isnoise == 1):
        param_values[2] = AddNoise(param_values[2],3)
        param_values[4] = AddNoise(param_values[4],3)
        param_values[6] = AddNoise(param_values[6],6)
    #print(weight.type)    
    ...
    savepath = "/home/hc218/Zichen_Fan/improved-gan/inception_score/data_fake_noise/"
